...
fig3_2 <- ggplot(df3_2, aes(x = v, y = n)) +
  geom_rect(mapping = aes(xmin = -1, xmax = 18, ymin = 3, ymax = 7),
            fill = NA,
            colour = "black") +
  geom_segment(mapping = aes(x = 1, y = 6, xend = 16, yend = 6)) +
  geom_segment(mapping = aes(x = 1, y = 4, xend = 16, yend = 4)) +
  ...
  geom_segment(mapping = aes(x = quantile(df3_2$v)[[1]], y = 4, 
                             xend = quantile(df3_2$v)[[3]], yend = 4),
               arrow = grid::arrow(ends = "both",
                                   type = "closed", 
                                   length = unit(0.05, "inches"))) +
  geom_segment(mapping = aes(x = quantile(df3_2$v)[[3]], y = 4, 

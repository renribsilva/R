...
fig3_4 <- ggplot(df3_4) +
  ...
  scale_y_continuous(breaks = quantile(df3_4$v),
  ...
  scale_x_continuous(expand = expansion(mult = c(0.2,0.5)),
                     sec.axis = sec_axis(trans = "identity")) +
  geom_segment(mapping = aes(x = 0.75, 
                             y = quantile(df3_4$v)[[1]], 
                             xend = 0.75, 
                             yend = quantile(df3_4$v)[[2]]),
               arrow = grid::arrow(length = unit(0.05, "inches"),
                                   ends = "both", type = "closed")) +
  geom_segment(mapping = aes(x = 0.75, 
                             y = quantile(df3_4$v)[[2]], 
                             xend = 0.75, 
                             yend = quantile(df3_4$v)[[4]]),
               arrow = grid::arrow(length = unit(0.05, "inches"),
                                   ends = "both", type = "closed")) +
  geom_segment(mapping = aes(x = 0.75, 
                             y = quantile(df3_4$v)[[4]], 
                             xend = 0.75, 
                             yend = quantile(df3_4$v)[[5]]),
               arrow = grid::arrow(length = unit(0.05, "inches"),
                                   ends = "both", type = "closed")) +
  geom_text(data = data.frame(x = c(rep(0.9, times = 3)),
                              y = c(quantile(df3_4$v)[[1]]+(quantile(df3_4$v)[[2]]-quantile(df3_4$v)[[1]])/2,
